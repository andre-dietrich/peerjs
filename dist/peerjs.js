...
class $3cabb799c2437cbe$export$89e6bb5ad64bf4a {
    /** Returns a PeerConnection object set up correctly (for data, media). */ startConnection(options = {}) {
        ...
        if (this.connection.type === (0, $edbcf3072a01d580$export$3157d57b4135e3bc).Media) {
    ...
    cleanup() {
        ...
        try {
            ...
            if (peerConnection.getTransceivers) peerConnection.getTransceivers().forEach((transceiver)=>{
                // For server-side connections, we shouldn't stop the sender tracks as they might
                // be shared with other client connections
                if (transceiver.sender && transceiver.sender.track) // Only stop sender tracks if this is a client connection
                {
                    if (!isServer) transceiver.sender.track.stop();
                }
                // We can always stop receiver tracks from clients
                if (transceiver.receiver && transceiver.receiver.track) // Only stop receiver tracks if we know they're not shared
                {
                    if (transceiver.direction === "sendonly" || !isServer) transceiver.receiver.track.stop();
                }
            });
            else if (peerConnection.getSenders) peerConnection.getSenders().forEach((sender)=>{
                if (sender.track && !isServer) sender.track.stop();
            });
        } catch (err) {
            (0, $d3fc3aacca52312c$export$2e2bcd8739ae039).warn("Error stopping tracks", err);
        }
        ...
        if (peerConnection.oniceconnectionstatechange) peerConnection.oniceconnectionstatechange = null;
        ...
        if (dataChannel) {
            try {
                ...
                (0, $d3fc3aacca52312c$export$2e2bcd8739ae039).warn("Error closing data channel", err);
            }
            // Clear reference
            this.connection.dataChannel = null;
        }
        // Close peer connection if not already closed
        if (peerConnection.signalingState !== "closed") try {
            peerConnection.close();
        } catch (err) {
            (0, $d3fc3aacca52312c$export$2e2bcd8739ae039).warn("Error closing PeerConnection", err);
        }
    ...
    async _makeOffer() {
        ...
        try {
            ...
            try {
                ...
                if (this.connection.options.sdpTransform) try {
                    const transformResult = this.connection.options.sdpTransform(offer.sdp);
                    // Only use the result if it's a non-empty string
                    if (typeof transformResult === "string" && transformResult.length > 0) {
                        processedSdp = transformResult;
                        (0, $d3fc3aacca52312c$export$2e2bcd8739ae039).log("Applied sdpTransform to offer");
                    } else (0, $d3fc3aacca52312c$export$2e2bcd8739ae039).warn("sdpTransform returned invalid SDP, using original");
                } catch (transformError) {
                    provider.emitError((0, $edbcf3072a01d580$export$9547aaa2e39030ff).WebRTC, transformError);
                    (0, $d3fc3aacca52312c$export$2e2bcd8739ae039).error("Error in sdpTransform function:", transformError);
                }
    ...
    async _makeAnswer() {
        ...
        try {
            ...
            try {
                ...
                if (this.connection.options.sdpTransform) try {
                    const transformResult = this.connection.options.sdpTransform(answer.sdp);
                    // Only use the result if it's a non-empty string
                    if (typeof transformResult === "string" && transformResult.length > 0) {
                        processedSdp = transformResult;
                        (0, $d3fc3aacca52312c$export$2e2bcd8739ae039).log("Applied sdpTransform to answer");
                    } else (0, $d3fc3aacca52312c$export$2e2bcd8739ae039).warn("sdpTransform returned invalid SDP, using original");
                } catch (transformError) {
                    provider.emitError((0, $edbcf3072a01d580$export$9547aaa2e39030ff).WebRTC, transformError);
                    (0, $d3fc3aacca52312c$export$2e2bcd8739ae039).error("Error in sdpTransform function:", transformError);
    ...
    /** Handle an SDP. */ async handleSDP(type, sdp) {
        ...
        try {
            ...
            (0, $d3fc3aacca52312c$export$2e2bcd8739ae039).log(`Set remote description ${type} successfully`);
            if (type === "OFFER") await this._makeAnswer();
        } catch (err) {
            const errorMessage = `Failed to set remote ${type} SDP: ${err.message}`;
            (0, $d3fc3aacca52312c$export$2e2bcd8739ae039).error(errorMessage, err);
            // Categorize error for better handling
            if (err.name === "InvalidStateError") provider.emitError((0, $edbcf3072a01d580$export$9547aaa2e39030ff).WebRTC, `Invalid signaling state: ${peerConnection.signalingState}. ${errorMessage}`);
            else if (err.name === "InvalidAccessError") provider.emitError((0, $edbcf3072a01d580$export$9547aaa2e39030ff).WebRTC, `Invalid SDP format or unsupported configuration. ${errorMessage}`);
            else provider.emitError((0, $edbcf3072a01d580$export$9547aaa2e39030ff).WebRTC, errorMessage);
            this.connection.close();
        }
    }
...
class $0bc12d7ac26e1491$export$4a84e95a2324ac29 extends (0, $f06f075a81a8b63e$export$23a2a68283c24d80) {
    ...
    constructor(peerId, provider, options){
        super(peerId, provider, options), this._localStream = null;
